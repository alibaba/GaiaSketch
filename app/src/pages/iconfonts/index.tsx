...
export function Iconfont(props: IIconfontProps) {
    ...
    const [customColor, setCustomColor] = useState<any>("#000000");
    ...
    useEffect(() => {
        window.onDidDeleteIconfonts = () => {
            setSelectedLibraryInfo(null);
            setAreaContent(null);
            setOptions([]);
            setLoading(true);
            setTimeout(() => {
                window.postMessage("getIconfontLibraries", true);
            }, 100);
        };
    ...
    useEffect(() => {
        setTimeout(() => {
            window.postMessage("getIconfontLibraries");
        }, 100);
    }, []);

    useEffect(() => {
        window.onDidGetIconfonts = (data: any) => {
            setLoading(false);
                                        ...
                                        onDragStart={(e) => {
                                            ...
                                            if (toggleCustom) {
                                                e.dataTransfer.setData(
                                                    "text/plain",
                                                    `${
                                                        selectedLibraryInfo.projectID
                                                    }/${encodeURIComponent(
                                                        link.fullName
                                                    )}/${customColor}/${JSON.stringify(customSize)}`
                                                );
                                            } else {
                                                e.dataTransfer.setData(
                                                    "text/plain",
    ...
    }, [groups, toggleCustom, customSize, customColor]);
    ...
    useEffect(() => {
        if (selectedLibraryInfo) {
            if (selectedLibraryInfo.libraryID) {
                setLoading(true);
                setTimeout(() => {
                    window.postMessage("getIconfonts", selectedLibraryInfo, forceReload);
    ...
    useEffect(() => {
        window.onDidGetLatestSelectedLibrary = (data: any) => {
            if (data && data.libraryInfo && data.type === BarType.Iconfont) {
                setSelectedLibraryInfo(data.libraryInfo);
            }
        };
        window.postMessage("getLatestSelectedLibrary", BarType.Iconfont);
    }, []);

    useEffect(() => {
        window.onDidUpdateLibrary = (data: any) => {
            if (data && data.type === BarType.Iconfont && data.libraryInfo) {
    ...
    useEffect(() => {
        window.onDidImportIconfonts = (data: any) => {
        ...
        >
                    ...
                    <IconButton
                        ...
                        onClick={() => {
                            ...
                            if (selectedLibraryInfo) {
                                setLoading(true);
                                setTimeout(() => {
                                    window.postMessage("getIconfonts", selectedLibraryInfo, true);
                                }, 100);
                            } else {
                                setTimeout(() => {
                                    window.postMessage("getIconfontLibraries", true);
                            ...
                            items: [
                                {
                                    ...
                                    onClick: () => {
                                        setTimeout(() => {
                                            window.postMessage("importIconfonts");
                                        }, 100);
                                    }
                                },
                                {
            ...
            >
                {showCustom ? (
                    <>
                        <Stack horizontal={true} styles={{ root: { height: "100%" } }}>
                            <TextField
                                disabled={!toggleCustom}
                                value={customSize["width"]}
                                styles={{ root: { minWidth: 35, maxWidth: 50 } }}
                                onChange={(event: any, newValue?: string) => {
                                    setCustomSize(
                                        Object.assign({}, { ...customSize }, { width: newValue })
                                    );
                                }}
                            />
                            <IconButton
                            ...
                            <TextField
                                ...
                                onChange={(event: any, newValue?: string) => {
                                    setCustomSize(
                                        Object.assign({}, { ...customSize }, { height: newValue })
                                    );
                                }}
                            />
                        </Stack>
                ...
                <TextField
                    disabled={!toggleCustom}
                    styles={{
                        prefix: { backgroundColor: `${customColor}`, width: 10 },
                        icon: { color: "white" }
                    }}
                    prefix={"  "}
                ...
                >
                    <SketchPicker
                        ...
                        onChange={(color: ColorResult) => {
                            if (color) {
                                let tinyColor: any = TinyColor(color.rgb);
                                if (tinyColor.isValid()) {
                                    if (tinyColor.getAlpha() != 1) {
                                        setCustomColor(tinyColor.toHex8String());
                                    } else {
                                        setCustomColor(tinyColor.toHexString());
                                    }
                                }
                            }
                        }}
