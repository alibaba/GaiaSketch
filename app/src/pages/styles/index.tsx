...
export function Style(props: IStyleProps) {
  ...
  useEffect(() => {
    window.onDidDeleteStyles = () => {
      setSelectedLibraryInfo(null);
      setAreaContent(null);
      setOptions([]);
      setLoading(true);
      setTimeout(() => {
        window.postMessage("getStyleLibraries", true);
      }, 100);
    };
  ...
  useEffect(() => {
    setTimeout(() => {
      window.postMessage("getStyleLibraries");
    }, 100);
  }, []);

  useEffect(() => {
    window.onDidGetStyles = (data: any) => {
      setLoading(false);
      setAreaContent(
        <Nav
          styles={{
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        {link.leafName && link.leafName !== link.name ? (
                          <Text
                            draggable={false}
                            styles={{
                              root: {
  ...
  useEffect(() => {
    if (selectedLibraryInfo) {
      if (selectedLibraryInfo.libraryID) {
        setLoading(true);
        setTimeout(() => {
          window.postMessage("getStyles", selectedLibraryInfo, forceReload);
  ...
  useEffect(() => {
    window.onDidUpdateLibrary = (data: any) => {
      if (data && data.type === BarType.Style && data.libraryInfo) {
  ...
  useEffect(() => {
    window.onDidImportStyles = (data: any) => {
          ...
          <IconButton
            ...
            onClick={() => {
              ...
              if (selectedLibraryInfo) {
                setLoading(true);
                setTimeout(() => {
                  window.postMessage("getStyles", selectedLibraryInfo, true);
                }, 100);
              } else {
                setTimeout(() => {
                  window.postMessage("getStyleLibraries", true);
              ...
              items: [
                {
                  ...
                  onClick: () => {
                    setTimeout(() => {
                      window.postMessage("importStyles");
                    }, 100);
                  },
                },
                {
                  ...
                  onClick: () => {
                    setTimeout(() => {
                      window.postMessage("deleteStyles", selectedLibraryInfo);
                    }, 100);
                  },
                },
                {
