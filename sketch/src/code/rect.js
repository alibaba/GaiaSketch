...
export function containRect(rect1, rect2) {
    return (
        rect2.x + 2 >= rect1.x &&
        rect2.x + rect2.width - 2 <= rect1.x + rect1.width &&
        rect2.y >= rect1.y - 2 &&
        rect2.y + rect2.height - 2 <= rect1.y + rect1.height
    );
}

export function intersect(rect1, rect2) {
    return !(
        rect2.x + 2 >= rect1.x + rect1.width ||
        rect2.x + rect2.width - 2 <= rect1.x ||
        rect2.y >= rect1.y + rect1.height - 2 ||
        rect2.y + rect2.height - 2 <= rect1.y
    );
}

export function isRectEqual(rect1, rect2) {
    return Math.abs(rect1.width - rect2.width) <= 2 && Math.abs(rect1.height - rect2.height) <= 2;
}
...
export function isColumn(layers) {
    ...
    for (let i = 0; i < compares.length && i + 1 < compares.length; i++) {
        ...
        let column = rect1.y + rect1.height - 2 <= rect2.y || rect2.y + rect2.height - 2 <= rect1.y;
        if (!column) {
            return false;
        }
    }
...
export function isRow(layers) {
    ...
    for (let i = 0; i < compares.length && i + 1 < compares.length; i++) {
        let rect1 = compares[i];
        let rect2 = compares[i + 1];
        let row = rect1.x + rect1.width - 2 <= rect2.x || rect2.x + rect2.width - 2 <= rect1.x;
        if (!row) {
            return false;
        }
    }
...
export function isTextColumn(layers) {
    let compares = [...layers];
    stable.inplace(compares, (a, b) => {
        if (a.y < b.y) {
            return -1;
    ...
    if (hasText) {
        for (let i = 0; i < compares.length && i + 1 < compares.length; i++) {
            let rect1 = compares[i];
            let rect2 = compares[i + 1];
            let column = rect1.y + rect1.height - 4 <= rect2.y || rect2.y + rect2.height - 4 <= rect1.y;
            if (!column) {
                return false;
            }
        }
...
export function isTextRow(layers) {
    let compares = [...layers];
    stable.inplace(compares, (a, b) => {
        if (a.x < b.x) {
            return -1;
    ...
    if (hasText) {
        for (let i = 0; i < compares.length && i + 1 < compares.length; i++) {
            let rect1 = compares[i];
            let rect2 = compares[i + 1];
            let row = rect1.x + rect1.width - 4 <= rect2.x || rect2.x + rect2.width - 4 <= rect1.x;
            if (!row) {
                return false;
            }
        }
...
export function hasOtherIntersect(parentLayers, layers) {
    let ur = unionRect(layers);
    for (let index3 = 0; parentLayers && index3 < parentLayers.length; index3++) {
        const element = parentLayers[index3];
        const elementFrame = { x: element.x, y: element.y, width: element.width, height: element.height };
        if (
            !findLayer(layers, element.id) &&
            intersect(ur, elementFrame) &&
            (!containRect(elementFrame, ur) || !containRect(elementFrame, ur))
        ) {
            return true;
        }
    }
