    ...
    constructor(props) {
        ...
        if (props.layer.image !== undefined && !containFills) {
            if (this.source == undefined) {
                ...
                fs.writeFileSync(path.join(assetsPath, `${md5String}.png`), base64String, {
                    encoding: "base64",
                });
                this.source = `data:image/png;base64,${base64String}`;
            }
        } else {
            if (props.layerToImage || containFills) {
                ...
                if (originRect.width != this.style.width || originRect.height != this.style.height) {
                ...
                // );
                this.source = `data:image/png;base64,${fs.readFileSync(layerToBase64(props.layer), { encoding: "base64" })}` ;
                props.layer.frame = new Rectangle(originRect.x, originRect.y, originRect.width, originRect.height);
                ...
                } else if (needToHandleBorder && needToHandleBorder.fillType === "Color") {
                    this.style.borderColor = normalizeColor(needToHandleBorder.color);
                    this.style.borderWidth = needToHandleBorder.thickness;
                    this.style.borderStyle = "solid";
                ...
                if (props.parent && (targetLayer.style.shadows.length > 0 || needToHandleBorder)) {
                    props.parent.notContainOverfow = true;
                }
            }
        }
        this.resizeMode = "contain";
        //logger.log(`layer.name=${props.layer.name}, resizeMode=${this.resizeMode}`);
