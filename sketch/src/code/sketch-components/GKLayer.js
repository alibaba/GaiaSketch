...
export default class GKLayer {
  ...
  constructor(props) {
    ...
    //console.log(`props.layer.id=${props.layer.id}, layer name=${props.layer.name}`);
    // //console.log(`candidatesForMasking=${props.layer.sketchObject.closestClippingLayer()}`);
    ...
    // console.log(`props.layer.name=${props.layer.name}`);
    // //console.log(`origin style=${JSON.stringify(this.style)}`);
    // if (props.layer.type == "Text") {
    //   console.log(`props.layer=`, props.layer);
    // }
    if (props.scale) {
      if (!props.layer.sketchObject.hasFixedWidth()) {
        this.style.width = Math.floor(
          this.style.width * (props.scale.to.width / props.scale.from.width)
        );
      }
      if (!props.layer.sketchObject.hasFixedHeight()) {
        this.style.height = Math.floor(
          this.style.height * (props.scale.to.height / props.scale.from.height)
        );
      }
      ...
      if (!props.layer.sketchObject.hasFixedLeft()) {
        if (props.layer.sketchObject.hasFixedWidth()) {
          ...
          this.style.left = Math.floor(
            this.style.left * (props.scale.to.width / props.scale.from.width)
          );
        }
      }
      if (!props.layer.sketchObject.hasFixedTop()) {
        if (props.layer.sketchObject.hasFixedHeight()) {
          ...
          this.style.top = Math.floor(
            this.style.top * (props.scale.to.height / props.scale.from.height)
          );
        }
      }
    }
    //console.log(`modified style=${JSON.stringify(this.style)}`);

    //TODO: this.style.type = this.type; for test
    // this.style.type = this.type;

    // console.log(`layer=${this.name}, opacity=${props.layer.style.opacity}`);
    ...
    if (this.type == "Text") {
      this.style.textAlign =
        props.layer.alignment || props.layer.style.alignment;
    }
    ...
    if (props.layer.style !== undefined) {
        ...
        for (let i = props.layer.style.fills.length - 1; i >= 0; i--) {
          ...
          if (fi.enabled) {
            if (fi.fillType === sketch.Style.FillType.Color) {
              ...
              if (
                fi.gradient.gradientType === sketch.Style.GradientType.Linear
              ) {
                ...
                linearString2 += `'${angle}'`;
                  ...
                  if (color.length > 7) {
                    ...
                    color = `#${alphaString}${color.substring(
                      1,
                      color.length - 2
                    )}`;
                  }
      ...
      if (props.layer.sketchObject.rotation() > 0) {
        this.style.transform = `rotate(${props.layer.sketchObject.rotation()}deg)`;
      }
      ...
      // 处理border
      // //console.log(`props.layer.style.sketchObject.hasEnabledBorder() = ${props.layer.style.sketchObject.hasEnabledBorder()}`)
      if (props.layer.style.sketchObject.hasEnabledBorder()) {
        if (this.type !== "Group") {
  ...
  extractBorderStyle(layer) {
    if (layer.type === "Shape" || layer.type === "ShapePath") {
      ...
      if (
        ...
        (nativeObject.class() == "MSShapeGroup" &&
          nativeObject.layers()[0].class() == "MSRectangleShape")
      ) {
        let targetObject;
        // //console.log(`layer.name=${layer.name} is ${nativeObject.class()}`);
        ...
        // radius
        if (points.length == 4) {
          let radiusTopLeft = Math.round(
              points.objectAtIndex(0).cornerRadius()
            ),
            radiusTopRight = Math.round(points.objectAtIndex(1).cornerRadius()),
            radiusBottomRight = Math.round(
              points.objectAtIndex(2).cornerRadius()
            ),
            radiusBottomLeft = Math.round(
              points.objectAtIndex(3).cornerRadius()
            );
          if (
            ...
            this.style.borderRadius = Math.min(
              radiusTopLeft,
              this.style.width / 2,
              this.style.height / 2
            );
        ...
        // 默认是个圆形，这个后续可能需要改下
        // //console.log(`layer.name=${layer.name} is MSOvalShape`);
        this.style.borderRadius = this.style.width / 2;
      }
    }
