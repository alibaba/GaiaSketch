...
import { containSpread, copyFolderAsync, getURLFromPath, isAllShapeOrImages } from "./helper";
        ...
        downloadLibrary(Token, projectID)
            .then((data) => {
                if (data.libraryFolder && data.fileName) {
                    let content = fs.readFileSync(path.join(data.libraryFolder, data.fileName), { encoding: "binary" });
                    webContents.executeJavaScript(
                ...
                if (document) {
                    if (document.path) {
                        document.save(
                            ...
                            (error) => {
                                layerToMeasure(params, document, webContents, context, document.path, filePath);
                            }
                        );
                    } else {
                        let newPath = path.join(os.tmpdir(), String(NSUUID.UUID().UUIDString()) + ".sketch");
                        document.save(newPath, { saveMode: sketch.Document.SaveMode.SaveTo }, (error) => {
                            layerToMeasure(params, document, webContents, context, newPath, filePath);
                        });
                    }
                }
            } else {
...
export function layerToMeasure(options, document, webContents, context, documentPath, saveToPath) {
...
function doExport(context, directory, data, options, documentPath) {
    return new Promise((resolve, reject) => {
        ...
        if (directory) {
            ...
            targetPath = path.join(os.tmpdir(), "gaia-export-measure", String(NSUUID.UUID().UUIDString()));
        }
        ...
        copyFolderAsync(path.join(resourcesPath, "export-measure"), targetPath)
            .then(() => {
                ...
                let fromPath = decodeURIComponent(documentPath || sketch.getSelectedDocument().path);
                fs.copyFileSync(fromPath, toPath);
                doc = NSDocumentController.sharedDocumentController().openDocumentWithContentsOfURL_display_error(
                ...
                modifiedArtboards &&
                    modifiedArtboards.forEach((modifiedArtboard) => {
                        let artboardDataPath = path.join(artboardsFolder, String(modifiedArtboard.id) + ".js");
                        fs.writeFileSync(
                            artboardDataPath,
                            `window['${String(modifiedArtboard.id)}']=` + JSON.stringify(modifiedArtboard)
                        );
                        scripts.push(`<script src="artboards/${String(modifiedArtboard.id) + ".js"}"></script>`);
                        pngs.push({
      ...
      `;
                ...
                if (options.allowDesignToken) {
                    writeContent += `window['__TOKENS__'] = ${JSON.stringify(options.tokens)};`;
                }

                if (slicesInfo) {
                    writeContent += `window['__SLICES_INFOS__'] = ${JSON.stringify(slicesInfo)};`;
                }
                try {
                    fs.rmdirSync(path.join(targetPath, "tmps"));
                } catch (error) {}
                fs.writeFileSync(path.join(targetPath, "data.js"), writeContent);
                let indexContent = fs.readFileSync(path.join(targetPath, "index.html"), { encoding: "utf8" });
...
function modifyArtboards(doc, autoCut, slicesInfo, slicesFolder, artboards) {
    ...
    returnArtboards &&
        returnArtboards.forEach((artboard) => {
            if (artboard && artboard.layers && artboard.layers.length == 1 && artboard.layers[0].type == "Group") {
    ...
    returnArtboards &&
        returnArtboards.forEach((artboard) => {
            detachAllSymbolInstance(autoCut, slicesInfo[`${artboard.id}`], slicesFolder, artboard);
        });
...
function detachAllSymbolInstance(autoCut, slicesInfo, slicesFolder, layer) {
    ...
    if (currentLayer.exportFormats && currentLayer.exportFormats.length > 0) {
        ...
        if (autoCut && isAllShapeOrImages(currentLayer) && !containSpread(currentLayer, currentLayer.frame)) {
            let scales = ["2", "3"];
            let layerName = generateCombineLayerName(currentLayer);
            scales.forEach((scale) => {
                let dir = path.join(slicesFolder, "auto", layerName, String(scale) + "x");
                if (!fs.existsSync(path.join(dir, "index.png"))) {
                    ...
                    subDirs &&
                        subDirs.forEach((filename) => {
                            ...
                            fs.renameSync(path.join(dir, filename), path.join(dir, "index.png"));
                        });
...
function exportAllFormats(slicesInfo, slicesFolder, layer) {
    if (layer && !layer.hidden) {
        if (layer.exportFormats && layer.exportFormats.length > 0) {
            for (let index = 0; index < layer.exportFormats.length; index++) {
                ...
                if (format && format.size && String(format.size).endsWith("x")) {
                    ...
                    let dir = path.join(slicesFolder, "preset", layerName, String(format.size));
                    let targetPath = path.join(dir, `index.${format.fileFormat}`);
                    if (!fs.existsSync(targetPath)) {
                        sketch.export(layer, {
                            "use-id-for-name": true,
                            formats: format.fileFormat,
                            scales: String(format.size).substring(0, String(format.size).length - 1),
                            trimmed: false,
                            output: dir,
                            "group-contents-only": true,
                            overwriting: true,
        ...
        } else if (layer.type == "Shape" || layer.type == "ShapePath" || layer.type == "Text") {
            if (layer.name && (layer.name.indexOf("token:") != -1 || layer.name.indexOf("token：") != -1)) {
