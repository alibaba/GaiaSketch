...
export function registerComponentIPC(context, webContents) {
    ...
    webContents.on("getComponents", async (libraryInfo, forceReload) => {
        // logger.log(`start get components = `, libraryInfo);
        setLatestSelectedLibrary(Component, libraryInfo);
        let selectedLibrary = await getSelectedLibrary(Component, libraryInfo);
        if (selectedLibrary) {
            let symbolMasters = [];
                ...
                for (let j = symbols.length - 1; j >= 0; j--) {
                    ...
                    if (match) {
                        ...
                        if (splits.length > 0) {
                            ...
                            let base64String = fs.readFileSync(path.join(screenshotsFolder(), layer.id + "@2x.png"), {
                                encoding: "base64",
                            });
            ...
            webContents.executeJavaScript(`onDidGetComponents(${JSON.stringify({ groups: [] })})`);
        }
    });
}
                    ...
                    if (splitArray && splitArray.length === 3) {
                        ...
                        for (let i = 0; i < sketchLibraries.length; i++) {
                            ...
                            if (libDocument.id === libraryID) {
                                ...
                                let symbolReferences = sketchLibrary.getImportableSymbolReferencesForDocument(document);
