      ...
      .then(({ canceled, filePaths }) => {
        if (!canceled && filePaths && filePaths.length > 0) {
          ...
          if (fs.existsSync(filePath) && path.extname(filePath) === ".sketch") {
            ...
            webContents.executeJavaScript(
              `onDidImportStyles(${JSON.stringify(libraryInfo)})`
            );
          }
        } else {
  ...
  webContents.on("getStyles", async (libraryInfo, forceReload) => {
    try {
      ...
      if (selectedLibrary) {
        ...
        webContents.executeJavaScript(
          `onDidGetStyles(${JSON.stringify({ groups: symbolMasters })})`
        );
      } else {
        webContents.executeJavaScript(
          `onDidGetStyles(${JSON.stringify({ groups: [] })})`
        );
      }
    } catch (e) {
      console.log(e);
      ...
      for (let i = 0; i < document.selectedLayers.layers.length; i++) {
        ...
        if (selectedLayer.type === layer.type) {
          ...
          if (position == -1) {
            ...
            let tokenName = selectedLayer.name.substring(
              position + 6,
              selectedLayer.name.length - 1
            );
            overwriteToken(layerName, tokenName, layer.name).then(
              (overwrite) => {
                selectedLayer.name = layerName;
                if (overwrite) {
                  selectedLayer.name =
                    selectedLayer.name + `(token:${layer.name})`;
                } else {
                  selectedLayer.name =
                    selectedLayer.name + `(token:${tokenName} | ${layer.name})`;
                }
              }
            );
            if (
