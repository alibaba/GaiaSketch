...
import { onComponentDestroy, registerComponentIPC } from "./libraries/component";
...
import { onManagementDestroy, registerManagementIPC } from "./libraries/management";
import { logger, isProduction } from "./logger";

const webviewIdentifier = "gaia.sketch.webview";

export function onManualOpenPanel(context) {
    Settings.setSettingForKey("manualclosed", undefined);
    onOpenPanel(context);
}

export function onOpenPanel(context, barType) {
    if (!barType) {
        let lastManualClosed = Settings.settingForKey("manualclosed");
        if (lastManualClosed) {
            return;
        }
    }
    ...
    browserWindow.on("closed", () => {
        Settings.setSettingForKey("manualclosed", true);
    });
...
export function onShutdown(context, allWindowClosed) {
    const existingWebview = getWebview(webviewIdentifier);
    if (existingWebview) {
...
export function onCloseDocument(context) {
    ...
    for (let index = 0; index < count; index++) {
        ...
        if (element.isKindOfClass(NSClassFromString("MSDocumentWindow")) && element.isVisible()) {
            instanceCount++;
        }
    }
    if (Number(instanceCount - 1) <= 0) {
        onShutdown(context, !Settings.settingForKey("manualclosed"));
    }
}
